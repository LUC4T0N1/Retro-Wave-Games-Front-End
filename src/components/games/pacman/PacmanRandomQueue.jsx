...
function PacmanRandomQueue({ socket }) {
  ...
  useEffect(() => {
    ...
    socket.on('pacman-game-start', onStart);
    return () => {
      socket.off('pacman-game-start', onStart);
      socket.emit('pacman-leave-queue'); // safe to call even if not queued; server ignores unknown ids
    };
  }, [socket]);
  ...
  const joinQueue = (e) => {
    ...
    setError('');
    socket.emit('pacman-join-queue', { username: name });
    setPhase('queued');
  };

  const leaveQueue = () => {
    socket.emit('pacman-leave-queue');
    setPhase('form');
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: '1.5px solid rgba(255,224,102,0.30)',
        borderRadius: 6, padding: '38px 36px 32px',
        backdropFilter: 'blur(16px)',
        boxShadow: '0 0 40px rgba(255,224,102,0.10), inset 0 0 40px rgba(100,0,255,0.04)',
        display: 'flex', flexDirection: 'column', gap: 0,
        alignItems: 'center',
      }}>
        <div style={{
          fontFamily: "'VT323', monospace", fontSize: 13,
          color: '#ffe066', letterSpacing: '0.5em', marginBottom: 6,
          textShadow: '0 0 10px #ffe06688',
        }}>PACMAN</div>
        <div style={{
        ...
        {phase === 'form' && (
          <form onSubmit={joinQueue} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <button type="submit" style={btnStyle('#ffe066')}
              onMouseEnter={e => { e.currentTarget.style.background = '#ffe06618'; e.currentTarget.style.color = '#fff'; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'rgba(4,0,18,0.65)'; e.currentTarget.style.color = '#ffe066'; }}
            >
              FIND MATCH
            </button>
          </form>
        ...
        {phase === 'queued' && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
            <div style={{
              fontFamily: "'VT323', monospace", fontSize: 32, color: '#ffe066',
              letterSpacing: '0.3em', animation: 'glowPulse 2s ease-in-out infinite',
              textShadow: '0 0 16px #ffe066',
            }}>SEARCHING…</div>
...
const inputStyle = {
  ...
  background: 'rgba(255,224,102,0.05)', border: '1.5px solid rgba(255,224,102,0.25)',
...
export default PacmanRandomQueue;
