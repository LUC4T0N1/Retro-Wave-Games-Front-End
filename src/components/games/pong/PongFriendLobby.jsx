...
function PongFriendLobby({ socket }) {
  ...
  useEffect(() => {
    ...
    const onReady    = ({ side, room, opponent }) => { setGameData({ side, room, opponent }); setPhase('game'); };
    socket.on('pong-waiting',    onWaiting);
    socket.on('pong-room-ready', onReady);
    return () => { socket.off('pong-waiting', onWaiting); socket.off('pong-room-ready', onReady); };
  }, [socket]);
  ...
  const join = (e) => {
    ...
    setError('');
    socket.emit('pong-join-room', { room, username: name });
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow: '0 0 40px rgba(0,180,255,0.14), inset 0 0 40px rgba(100,0,255,0.04)',
...
export default PongFriendLobby;
